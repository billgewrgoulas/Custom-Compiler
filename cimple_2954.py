...
def subPars():
    ...
    if q[1] == 'call':
        callSub(pars, q[4], ins)
   
def callSub(pars, name, ins):
    global final, symTable
...
def callSubNoPars(name): #we could use callSub but whatever..
    ...
    final.append('  addi $sp, $sp, ' + str(e[0].fLength) + '\n')
    final.append('  jal ' + str(e[0].sQuad) + '\n')
    final.append('  addi $sp, $sp, -' + str(e[0].fLength) + '\n')
...
def delScope():
    ...
    printScope()
    fLength = symTable[-1].totalOffset
    genInstructions()
    symTable.pop()
    if not symTable:return
    symTable[-1].entities[-1].fLength = fLength
    nestingLevel -= 1
...
def validateSub(e, name):
    global line

    if e == 'err':
        print('error, subprogram %s out of scope at line %s ' % (name, line))
        exit(1)
    elif not e[0].type in ['procedure', 'function']:
        print('error, variable %s was used as a subprogram at line: %s' % (name, line))
        exit(1)
...
def validatePars(lst):
    ...
    for par in lst:
        if search(par, ['function', 'procedure']) != 'err' and not par.isdigit():
            print('error, subprogram name was used as a variable at line: %s' % (line))
            exit(1)
        if search(par, ['temp', 'par', 'var', 'global']) == 'err' and not par.isdigit():
            print('error, undefined argument %s at line: %s' % (par, line))
            exit(1)
...
def printScope():  #print the completed scope right before deletion
    global symTable

    for e in symTable[-1].entities:
        print(vars(e))
    print('scope name: %s' % symTable[-1].name )
    print('scope fLength: %s' % symTable[-1].totalOffset )
    print('scope level: %s' % symTable[-1].nestingLevel )
    print('=================================================================================')
...
def varlist(b): #declarations
    global token, line, symTable, offset
...
def subprograms():
    global token, line, symTable, final

    while token[0] == 'function_tk' or token[0] == 'procedure_tk':
        
        type = token[0].split('_')[0]
        token = lex()
        if token[0] == 'id_tk':
            name = token[1]
